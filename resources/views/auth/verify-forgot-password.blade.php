...
@section('content')
<!-- Logo Header -->
<div clacenter mb-8 animate-fade-in">
    ...
    <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-2 animate-slide-up">Password Reset Verification</h2>
...
<div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800 animate-slide-up" style="animation-delay: 0.3s;">
    <div class="flex items-center space-x-3">
        ...
        <div>
            ...
            <p class="text-sm text-blue-700 dark:text-blue-300" id="user-email">{{ session('forgot_password_email') ?? $email ?? 'your-email@example.com' }}</p>
        </div>
    </div>
</div>

<!-- OTP Form -->
<form action="{{ route('verify.password.post') }}" method="POST" id="verify-password-reset-form" class="space-y-6">
    @csrf

    <!-- Email (hidden field) -->
    <input type="hidden" name="email" value="{{ session('forgot_password_email') ?? $email ?? '' }}">
...
<div class="text-center animate-slide-up mt-6" style="animation-delay: 0.9s;">
